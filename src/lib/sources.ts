...
export function sourceStyle(
  ...
  switch (sourceType) {
    ...
    case "twitter":
      // X's own mark is pure black, invisible on our dark pill — same
      // problem discourse's mark has above, same fix: override the render
      // color, keep the real brand hex only for reference.
      return { label: "X", color: "#f7f7f5", logoPath: MARKS.x.path };
    case "manual":
    default:
...
/**
 * Extracts a tweet's numeric ID from any twitter.com/x.com status URL, or
 * null for anything else. Shared by the submit action (to classify a link
 * automatically) and the embed component (to know what to render) — one
